...
class WebMFile extends EventEmitter {
	...
	stream(timecode = 0) {
		...
		const self = this
		function* getNext() {
			let clusterId = self.getClusterIdAt(timecode)
			self.startTime = self.manifest.cues[clusterId].timecode
			while (clusterId < self.manifest.cues.length) {
				const cluster = self.manifest.cues[clusterId++]
				yield self.provider.getRange(cluster.start, cluster.end)
			}
		...
		class Reader extends Stream.Readable {
			constructor(opt) {
				super(opt)
				this.queue = new pq(1, Infinity)
				this.gen = getNext()
				this.queue.add(() => new Promise(resolve => {
					if (!self.finished) {
						const op = () => resolve(self._init)
						self.on('finish', op)
						this.on('destroy', () => self.removeListener('finish', op))
					} else resolve(self._init)
				}))
					.then(() => this.push(self._init))
				if (self.finished) self.startTime = self.manifest.cues[self.getClusterIdAt(timecode)].timecode
			}
			_read() {
				this.queue.add(() => this.gen.next().value)
	...
	getClusterIdAt(time) {
		...
		if (time < 0) return 0
		if (time > this.manifest.duration) return this.manifest.cues.length - 1
		let i = 1
